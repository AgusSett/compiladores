...
pcfMain xs = pretty "uint64_t* pcfmain()" <+> braces (nest 2 (line <> vsep (pcfMain' vs ++ [pretty "return 0;"])) <> line)
  where vals (IrVal _ _) = True
        vals _           = False
        vs = filter vals xs

pcfMain' :: [IrDecl] -> [Doc a]
pcfMain' [IrVal n t] = [pretty n <+> pretty "=" <+> ir2doc t <> semi, irPrint n]
...
prelude :: Doc a
prelude = pretty "#include <inttypes.h>" <> line <> pretty "extern void *pcf_mkclosure(void*, int, ...);"

irPrint :: String -> Doc a
irPrint x = pretty "pcf_print" <> tupled [pretty x] <> semi
